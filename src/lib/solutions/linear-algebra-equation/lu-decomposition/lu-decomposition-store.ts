...
import type { LUDecompositionResult } from './lu-decomposition';
...
const store = writable<{
	input: inputData;
	loading: boolean;
	result: LUDecompositionResult | null;
}>({
	input: {
...
const setResult = (result: LUDecompositionResult) => {
...
const fetchSolution = async () => {
	...
	const res = await fetch('/api/solution/linear/lu', {
		method: 'POST',
		headers: {
...
const createLUDecompositionStore = { subscribe, set, update, fetchSolution };

export default createLUDecompositionStore;
