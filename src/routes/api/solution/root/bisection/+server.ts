...
import { BisectionSearchSolver, RootOfEquationProblem } from '$lib/server/rootProblem';
...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	const problem = new RootOfEquationProblem(JSON.stringify(dataJson));
	...
	if (problemIdError || problemId == undefined) {
		const result = bisectionSearch(input.xStart, input.xEnd, input.errorFactor, input.func);
	...
	const problemSolved = new BisectionSearchSolver(problem);
