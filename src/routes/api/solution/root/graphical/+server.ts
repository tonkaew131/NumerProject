...
import { GraphicalMethodSolver, RootOfEquationProblem } from '$lib/server/rootProblem';
...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	if (problemIdError || problemId == undefined) {
		const result = graphicalMethod(input.xStart, input.xEnd, input.errorFactor, input.func);
	...
	const problemSolved = new GraphicalMethodSolver(problem);
