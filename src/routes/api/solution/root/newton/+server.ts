...
import { NewtonRaphsonSolver, OpenRootOfEquationProblem } from '$lib/server/rootProblem';
...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	if (problemIdError || problemId == undefined) {
		const result = newtonRaphson(input.xStart, input.errorFactor, input.func);
	...
	const problemSolved = new NewtonRaphsonSolver(problem);
