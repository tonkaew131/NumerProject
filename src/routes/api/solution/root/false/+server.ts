...
import { FalsePositionSolver, RootOfEquationProblem } from '$lib/server/rootProblem';
...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	if (problemIdError || problemId == undefined) {
		const result = falsePositionMethod(input.xStart, input.xEnd, input.errorFactor, input.func);
	...
	const problemSolved = new FalsePositionSolver(problem);
