...
import { IntegrateProblem, SimpsonRuleSolver } from '$lib/server/integrateProblem';
...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	if (problemIdError || problemId == undefined) {
		const result = simpsonRule(input.xStart, input.xEnd, input.func, input.n);
	...
	const problemSolved = new SimpsonRuleSolver(problem);
