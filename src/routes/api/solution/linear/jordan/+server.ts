...
import { GuassJordanSolver, LinearEquationProblem } from '$lib/server/linearEquationProblem';
import { guassJordanMethod } from '$lib/solutions/guassJordan';
...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	if (problemIdError || problemId == undefined) {
		const result = guassJordanMethod(input.matrixA, input.arrayB);
	...
	const problemSolved = new GuassJordanSolver(problem);
