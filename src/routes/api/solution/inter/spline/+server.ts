...
import {
	InterpolationOrderProblem,
	SplineInterpolationSolver
...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	const problem = new InterpolationOrderProblem(JSON.stringify(dataJson));
	...
	if (problemIdError || problemId == undefined) {
		const result = splineInterpolation(input.points, input.x, input.mode);
	...
	const problemSolved = new SplineInterpolationSolver(problem);
