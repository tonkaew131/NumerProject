<script lang="ts">
	...
	let result: resultType & BisectionResult;

	async function computeResult() {
		loading = true;

		const res = await fetch('/api/solution/root/bisection', {
			method: 'POST',
			headers: {
		...
		const resultIterations = bisectionSearch(
			Number(xStart),
			Number(xEnd),
			Number(errorFactor),
			formula
...
<svelte:head>
	...
	<meta name="description" content="Bisection search - Root of Eqation" />
</svelte:head>

<h3 class="text-center">🥹 Bisection search</h3>
